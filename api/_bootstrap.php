...
function api_lottery_history_payload(string $gameCode, array $input): array
{
    ...
    // Do not return the static snapshot before checking live/up-to-date data.
    // A snapshot is immutable and can keep history stuck on an old result
    // until the browser is refreshed. Live upstream/DB history must win.
    ...
    $list = [];
    // History starts with the latest finished round.
    // $lagStart is already the start of the latest completed period, so do NOT
    // subtract one more period for the first history item. The old formula
    // skipped the newest result and made the history appear stale until refresh.
    for ($i = 1; count($list) < $pageSize; $i++) {
        $drawTimeMs = $lagStart - (($offset + $i - 1) * $periodMs);
