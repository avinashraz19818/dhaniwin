...
import {
    l as be,
    a as ye
} from "./localUniformBit-BOGrcT1E.js";
import {
    c as xe,
    u as Ge,
    U as Pe,
    ...
    e as Te,
    t as Ce
} from "./BufferResource-DiDbkKt4.js";
import {
    S as W
...
import {
    ...
    S as Ue,
    d as we
} from "./RenderTargetSystem-BET8AZcL.js";
import "./index-BgAwOX9Q.js";
import "./colorToUniform-DUza8Qlq.js";
const G = W.for2d();
class H {
    start(e, t, r) {
        const s = e.renderer,
    ...
    execute(e, t) {
        ...
        const n = s.bindGroup.getBindGroup(t.bindGroup, r, 1),
            o = s.pipeline.getPipeline(this._geometry, r, G, t.topology);
...
class N {
    ...
    createGPUBuffer(e) {
        ...
        return e._updateID = 0, e._resourceId = ce("resource"), e.data && (de(e.data.buffer, t.getMappedRange(), e.data.byteOffset, e.data.byteLength), t.unmap()), e._gpuData[this._renderer.uid] = new Me(t), this._managedBuffers.add(e) && (e.on("update", this.updateBuffer, this), e.on("change", this.onBufferChange, this)), t
    }
    onBufferChange(e) {
...
class U {
    ...
    async _createDeviceAndAdaptor(e) {
        const t = await S.get().getNavigator().gpu.requestAdapter({
            ...
            r = ["texture-compression-bc", "texture-compression-astc", "texture-compression-etc2"].filter(i => t.features.has(i)),
...
class K {
    ...
    restoreRenderPass() {
        const e = this._renderer.renderTarget.adaptor.getDescriptor(this._renderer.renderTarget.renderTarget, !1, [0, 0, 0, 1], this._renderer.renderTarget.mipLevel, this._renderer.renderTarget.layer);
...
function Ee(a) {
    ...
    for (let r = 0; r < e.length; r++) {
        ...
        if (!P[s.data.type]) throw new Error(`[Pixi.js] WebGPU UniformBuffer: Unknown type ${s.data.type}`);
...
function Le(a, e) {
    const {
        size: t,
        align: r
    } = P[a.data.type], s = (r - t) / 4, i = a.data.type.indexOf("i32") >= 0 ? "dataInt32" : "data";
    return `
         v = uv.${a.data.name};
         ${e!==0?`offset += ${e};`:""}
...
class X {
    ...
    destroy() {
        var e;
        for (let t = 0; t < this._bindGroups.length; t++)(e = this._bindGroups[t]) == null || e.destroy();
...
class Z {
    ...
    getPipeline(e, t, r, s) {
        e._layoutKey || (Te(e, t.attributeData), this._generateBufferKey(e)), s || (s = e.topology);
        const i = ke(e._layoutKey, t._layoutKey, r.data, r._blendModeId, De[s]);
    ...
    _createPipeline(e, t, r, s) {
        const i = this._gpu.device,
            ...
            o = this._renderer.state.getColorTargets(r, this._colorTargetCount),
            u = this._stencilMode === C.RENDERING_MASK_ADD ? 0 : this._colorMask;
        for (let p = 0; p < o.length; p++) o[p].writeMask = u;
        const c = this._renderer.shader.getProgramData(t).pipeline,
            f = {
                vertex: {
        ...
        return this._depthStencilAttachment && (f.depthStencil = { ...this._stencilState,
    ...
    _updatePipeHash() {
        const e = ze(this._stencilMode, this._multisampleCount, this._colorMask, this._depthStencilAttachment, this._colorTargetCount);
...
class Oe {
    ...
    startRenderPass(e, t = !0, r, s, i = 0, n = 0) {
        ...
        if (n !== 0 && ((f = u.msaaTextures) != null && f.length)) throw new Error("[RenderTargetSystem] Rendering to array layers is not supported with MSAA render targets.");
        if (i > 0 && ((h = u.msaaTextures) != null && h.length)) throw new Error("[RenderTargetSystem] Rendering to mip levels is not supported with MSAA render targets.");
    ...
    getDescriptor(e, t, r, s = 0, i = 0) {
        ...
        const n = this._renderTargetSystem,
            ...
            u = e.colorTextures.map((h, p) => {
                ...
                } else g = this._renderer.texture.getGpuSource(h).createView({
                    dimension: "2d",
                    baseMipLevel: s,
                    mipLevelCount: 1,
                    baseArrayLayer: i,
                ...
                o.msaaTextures[p] && (m = g, g = this._renderer.texture.getTextureView(o.msaaTextures[p]));
    ...
    clear(e, t = !0, r, s, i = 0, n = 0) {
        ...
        const {
            gpu: o,
            encoder: u
        } = this._renderer, c = o.device;
        if (u.commandEncoder === null) {
            const h = c.createCommandEncoder(),
                p = this.getDescriptor(e, t, r, i, n),
                d = h.beginRenderPass(p);
    ...
    resizeGpuRenderTarget(e) {
        ...
        t.width = e.width, t.height = e.height, t.msaa && e.colorTextures.forEach((r, s) => {
            ...
            i == null || i.resize(r.source.width, r.source.height, r.source._resolution)
        })
    }
}
class J extends ve {
...
class Q {
    ...
    _createGPUProgramData(e) {
        const t = this._gpu.device,
            r = e.gpuLayout.map(i => t.createBindGroupLayout({
                entries: i
            })),
            s = {
                bindGroupLayouts: r
...
class ee {
    ...
    getColorTargets(e, t) {
        const r = _[e.blendMode] || _.normal,
            s = [],
            i = {
                format: "bgra8unorm",
                writeMask: 0,
...
const Ie = {
        ...
        upload(a, e, t, r = 0) {
            const s = a.resource,
                i = (a.pixelWidth | 0) * (a.pixelHeight | 0),
                n = s.byteLength / i;
    ...
    He = {
        ...
        upload(a, e, t, r = 0) {
            ...
            for (let o = 0; o < a.resource.length; o++) {
                const u = a.resource[o],
                    c = Math.ceil(s / n.blockWidth) * n.blockBytes;
                t.device.queue.writeTexture({
                    texture: e,
                    mipLevel: o,
                    origin: {
...
const re = {
        ...
        upload(a, e, t, r = 0) {
            ...
            if (globalThis.HTMLImageElement && s instanceof HTMLImageElement) {
                const u = S.get().createCanvas(s.width, s.height);
            ...
            const i = Math.min(e.width, a.resourceWidth || a.pixelWidth),
                n = Math.min(e.height, a.resourceHeight || a.pixelHeight),
...
class se {
    ...
    getPixels(e) {
        ...
        const {
            width: i,
            height: n
        } = t, o = s.getImageData(0, 0, i, n), u = new Uint8ClampedArray(o.data.buffer);
        return M.returnCanvasAndContext(r), {
            pixels: u,
            width: i,
            height: n
        }
...
class ie {
    ...
    contextChange(e) {
        ...
        const r = F({
            name: "graphics",
            bits: [fe, ge(this._maxTextures), be, O]
        });
        this.shader = new I({
            gpuProgram: r,
            resources: {
    ...
    execute(e, t) {
        ...
        for (let g = 0; g < u.instructionSize; g++) {
            ...
            if (m.topology !== d && (d = m.topology, c.setPipelineFromGeometryProgramAndState(o.geometry, s.gpuProgram, e.state, m.topology)), s.groups[1] = m.bindGroup, !m.gpuBindGroup) {
                const x = m.textures;
                m.bindGroup = L(x.textures, x.count, this._maxTextures), m.gpuBindGroup = i.bindGroup.getBindGroup(m.bindGroup, s.gpuProgram, 1)
            }
...
const qe = [...Ue, $, K, U, q, N, se, J, Q, ee, Z, j, Y, V],
    Ye = [...we, X],
    $e = [H, ne, ie],
    oe = [],
    ae = [],
    ue = [];
B.handleByNamedList(l.WebGPUSystem, oe);
B.handleByNamedList(l.WebGPUPipes, ae);
B.handleByNamedList(l.WebGPUPipesAdaptor, ue);
B.add(...qe, ...Ye, ...$e);
class nt extends me {
    constructor() {
        const e = {
