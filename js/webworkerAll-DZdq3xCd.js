...
import {
    c as q
} from "./canvasUtils-Dmnz5saj.js";
import {
    F as L
} from "./Filter-DmRrYMet.js";
import "./index-BgAwOX9Q.js";
const y = new k;
...
function G(x, e) {
    ...
    for (let r = 0; r < x.length; r++) {
        ...
        n != null && n.isCachedAsTexture ? e.matrix = y.copyFrom(n.textureOffsetInverseTransform).append(i.worldTransform) : n != null && n._parentCacheAsTextureRenderGroup ? e.matrix = y.copyFrom(n._parentCacheAsTextureRenderGroup.inverseWorldTransform).append(i.groupTransform) : e.matrix = i.worldTransform, e.addBounds(i.bounds)
    }
    return e.matrix = t, e
}

function S(x) {
    return typeof x.getCanvasFilterString == "function"
}
class W {
    constructor() {
...
class C {
    ...
    push(e) {
        ...
        if (this._savedStates.push({
            ...
            s.save(), s.setTransform(1, 0, 0, 1, 0, 0), s.beginPath(), s.rect(t.bounds.x * o, t.bounds.y * o, t.bounds.width * o, t.bounds.height * o), s.clip()
        } else t.useClip = !1;
        t.cssFilterString && (s.filter = a !== "none" ? `${a} ${t.cssFilterString}` : t.cssFilterString)
    }
    pop() {
    ...
    generateFilteredTexture({
        ...
        if (!(t != null && t.length) || t.every(d => !d.enabled)) return e;
        const r = [],
            i = 1;
        for (const d of t) {
            ...
            const T = d.getCanvasFilterString();
            if (T === null) {
                this._warnUnsupportedFilter(d);
                continue
            }
        ...
        const n = q.getCanvasSource(e);
        if (!n) return e;
        const s = e.frame,
            a = e.source._resolution ? ? e.source.resolution ? ? 1,
            o = s.width,
            l = s.height,
            f = O.getOptimalCanvasAndContext(o, l, a),
            {
                canvas: c,
                context: u
            } = f;
        ...
        const p = s.x * a,
            ...
            m = o * a,
            F = l * a;
        return u.drawImage(n, p, h, m, F, 0, 0, m, F), u.filter = "none", u.globalAlpha = 1, A(c, o, l, a)
    }
    _calculateFilterArea(e, t) {
        if (e.renderables ? G(e.renderables, t) : e.filterEffect.filterArea ? (t.clear(), t.addRect(e.filterEffect.filterArea), t.applyMatrix(e.container.worldTransform)) : e.container.getFastGlobalBounds(!0, t), e.container) {
            const r = e.container.renderGroup || e.container.parentRenderGroup,
                i = r == null ? void 0 : r.cacheToLocalTransform;
            i && t.applyMatrix(i)
        }
    }
    _warnUnsupportedFilter(e) {
        ...
        this._warnedFilterTypes.has(t) || this._warnedFilterTypes.add(t)
    }
    get alphaMultiplier() {
    ...
    _pushFilterFrame() {
        ...
        return e || (e = this._filterStack[this._filterStackIndex] = new W), this._filterStackIndex++, e
    }
    _popFilterFrame() {
...
class B {
    constructor(e) {
        ...
        }), this._globalFilterBindGroup = new z({}), this.renderer = e
    }
    ...
    push(e) {
        ...
        const o = i.bounds;
        if (this._calculateFilterArea(e, o), this._calculateFilterBounds(i, t.renderTarget.rootViewPort, a, s, 1), i.skip) return;
        const l = this._getPreviousFilterData(),
            f = this._findFilterResolution(s);
        let c = 0,
            u = 0;
        l && (c = l.bounds.minX, u = l.bounds.minY), this._calculateGlobalFrame(i, c, u, f, n.width, n.height), this._setupFilterTextures(i, o, t, l)
    }
    generateFilteredTexture({
        texture: e,
        filters: t
        ...
        const i = e.source,
            n = i.resolution,
            s = i.antialias;
        if (t.every(p => !p.enabled)) return r.skip = !0, e;
        const a = r.bounds;
        if (a.addRect(e.frame), this._calculateFilterBounds(r, a.rectangle, s, n, 0), r.skip) return e;
        const o = n;
        this._calculateGlobalFrame(r, 0, 0, o, i.width, i.height), r.outputRenderSurface = _.getOptimalTexture(a.width, a.height, r.resolution, r.antialias), r.backTexture = v.EMPTY, r.inputTexture = e, this.renderer.renderTarget.finishRenderPass(), this._applyFiltersToTexture(r, !0);
    ...
    getBackTexture(e, t, r) {
        const i = e.colorTexture.source._resolution,
            n = _.getOptimalTexture(t.width, t.height, i, !1);
        let s = t.minX,
            a = t.minY;
    ...
    applyFilter(e, t, r, i) {
        ...
        this._updateFilterUniforms(t, r, s, c, u, f, o, i);
        const p = e.enabled ? e : this._getPassthroughFilter();
        this._setupBindGroupsAndRender(p, t, n)
    }
    calculateSpriteMatrix(e, t) {
        const r = this._activeFilterData,
            ...
            n = t.worldTransform.copyTo(k.shared),
    ...
    _getPassthroughFilter() {
        return this._passthroughFilter ? ? (this._passthroughFilter = new $), this._passthroughFilter
    }
    ...
    _setupFilterTextures(e, t, r, i) {
        if (e.backTexture = v.EMPTY, e.inputTexture = _.getOptimalTexture(t.width, t.height, e.resolution, e.antialias), e.blendRequired) {
    ...
    _updateFilterUniforms(e, t, r, i, n, s, a, o) {
        const l = this._filterGlobalUniforms.uniforms,
            ...
            h = l.uGlobalFrame,
            m = l.uOutputTexture;
        a ? (f[0] = r.bounds.minX - i, f[1] = r.bounds.minY - n) : (f[0] = 0, f[1] = 0), f[2] = e.frame.width, f[3] = e.frame.height, c[0] = e.source.width, c[1] = e.source.height, c[2] = 1 / c[0], c[3] = 1 / c[1], u[0] = e.source.pixelWidth, u[1] = e.source.pixelHeight, u[2] = 1 / u[0], u[3] = 1 / u[1], p[0] = .5 * u[2], p[1] = .5 * u[3], p[2] = e.frame.width * c[2] - .5 * u[2], p[3] = e.frame.height * c[3] - .5 * u[3];
        const F = this.renderer.renderTarget.rootRenderTarget.colorTexture;
        h[0] = i * s, h[1] = n * s, h[2] = F.source.width * s, h[3] = F.source.height * s, t instanceof v && (t.source.resource = null);
        const d = this.renderer.renderTarget.getRenderTarget(t);
        this.renderer.renderTarget.bind(t, !!o), t instanceof v ? (m[0] = t.frame.width, m[1] = t.frame.height) : (m[0] = d.width, m[1] = d.height), m[2] = d.isRoot ? -1 : 1, this._filterGlobalUniforms.update()
    }
    ...
    _applyFiltersToTexture(e, t) {
        ...
        if (this._globalFilterBindGroup.setResource(r.source.style, 2), this._globalFilterBindGroup.setResource(e.backTexture.source, 3), s === a) n[s].apply(this, r, e.outputRenderSurface, t);
            ...
            const l = _.getOptimalTexture(i.width, i.height, o.source._resolution, !1);
            let f = l;
            for (let c = s; c < a; c++) {
                const u = n[c];
            ...
            n[a].apply(this, o, e.outputRenderSurface, t), _.returnTexture(l)
        }
    }
    _calculateFilterBounds(e, t, r, i, n) {
        ...
        for (let T = 0; T < o.length; T++) {
            ...
            if (m === -1 && (m = T), F = T, l = Math.min(l, g.resolution === "inherit" ? i : g.resolution), f += g.padding, g.antialias === "off" ? c = !1 : g.antialias === "inherit" && c && (c = r), g.clipToViewport || (h = !1), !!!(g.compatibleRenderers & s.type)) {
                p = !1;
                break
            }
            if (g.blendRequired && !(((d = s.backBuffer) == null ? void 0 : d.useBackBuffer) ? ? !0)) {
                X("Blend filter requires backBuffer on WebGL renderer to be enabled. Set `useBackBuffer: true` in the renderer options."), p = !1;
                break
            }
        ...
        if (h && a.fitBounds(0, t.width / i, 0, t.height / i), a.scale(l).ceil().scale(1 / l).pad((f | 0) * n), !a.isPositive) {
            e.skip = !0;
            return
        }
        e.antialias = c, e.resolution = l, e.blendRequired = u, e.firstEnabledIndex = m, e.lastEnabledIndex = F
    }
    _popFilterData() {
    ...
    _pushFilterData() {
        let e = this._filterStack[this._filterStackIndex];
        return e || (e = this._filterStack[this._filterStackIndex] = new Z), this._filterStackIndex++, e
    }
}
B.extension = {
