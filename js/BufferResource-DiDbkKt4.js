...
function O(e, t) {
    for (const a in e.attributes) {
        const s = e.attributes[a],
            f = t[a];
        f ? (s.format ? ? (s.format = f.format), s.offset ? ? (s.offset = f.offset), s.instance ? ? (s.instance = f.instance)) : _(`Attribute ${a} is not present in the shader, but is present in the geometry. Unable to infer attribute details.`)
    }
    U(e)
}
...
function U(e) {
    ...
    for (const n in a) {
        const r = a[n];
        s[r.buffer.uid] += y(r.format).stride
    }
    for (const n in a) {
        const r = a[n];
        r.stride ? ? (r.stride = s[r.buffer.uid]), r.start ? ? (r.start = f[r.buffer.uid]), f[r.buffer.uid] += y(r.format).stride
    }
}
const d = [];
d[c.NONE] = void 0;
...
class T {
    ...
    _systemCheck() {
        if (!S()) throw new Error("Current environment does not allow unsafe-eval, please use pixi.js/unsafe-eval module to enable support.")
    }
    ensureUniformGroup(t) {
        const a = this.getUniformGroupData(t);
    ...
    _initUniformGroup(t) {
        ...
        if (!s) {
            const f = Object.keys(t.uniformStructures).map(l => t.uniformStructures[l]),
                n = this._adaptor.createUboElements(f),
                r = this._generateUboSync(n.uboElements);
    ...
    syncUniformGroup(t, a, s) {
        const f = this.getUniformGroupData(t);
        t.buffer || (t.buffer = new g({
            data: new Float32Array(f.layout.size / 4),
            usage: m.UNIFORM | m.COPY_DST
        }));
        let n = null;
        return a || (a = t.buffer.data, n = t.buffer.dataInt32), s || (s = 0), f.syncFunction(t.uniforms, a, n, s), !0
    }
    updateUniformGroup(t) {
        if (t.isStatic && !t._dirtyId) return !1;
        t._dirtyId = 0;
        const a = this.syncUniformGroup(t);
...
function k(e, t, a, s) {
    ...
    for (let l = 0; l < e.length; l++) {
        ...
        for (let u = 0; u < h.length; u++)
            if (h[u].test(o.data)) {
                i = o.offset / 4, f.push(`name = "${b}";`, `offset += ${i-n};`, h[u][t] || h[u].ubo), x = !0;
                break
            }
        if (!x)
            if (o.data.size > 1) i = o.offset / 4, f.push(a(o, i - n));
            else {
                const u = s[o.data.type];
                i = o.offset / 4, f.push(`
                    v = uv.${b};
                    offset += ${i-n};
                    ${u};
    ...
    const r = f.join(`
`);
    return new Function("uv", "data", "dataInt32", "offset", r)
}
