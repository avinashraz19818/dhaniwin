    ...
    return Nt || (Nt = 1, (function(r) {
        ...
        r.getRowColCoords = function(e) {
            ...
            const t = Math.floor(e / 7) + 2,
                ...
                a = [n - 7];
            for (let s = 1; s < t - 1; s++) a[s] = a[s - 1] - u;
            ...
            const t = [],
                ...
                u = n.length;
            for (let a = 0; a < u; a++)
                for (let s = 0; s < u; s++) a === 0 && s === 0 || a === 0 && s === u - 1 || a === u - 1 && s === 0 || t.push([n[a], n[s]]);
            return t
        }
    })(x)), x
}
...
function ae() {
    return Pt || (Pt = 1, (function(r) {
        ...
        r.isValid = function(t) {
            ...
            for (let c = 0; c < n; c++) {
                a = s = 0;
                for (let d = 0; d < n; d++) a = a << 1 & 2047 | t.get(c, d), d >= 10 && (a === 1488 || a === 93) && u++, s = s << 1 & 2047 | t.get(d, c), d >= 10 && (s === 1488 || s === 93) && u++
            }
            return u * o.N3
        ...
        r.applyMask = function(t, n) {
            const u = n.size;
            for (let a = 0; a < u; a++)
                for (let s = 0; s < u; s++) n.isReserved(s, a) || n.xor(s, a, i(t, s, a))
    ...
    return Lt || (Lt = 1, (function(r) {
        ...
        r.mul = function(e, t) {
            ...
            for (let u = 0; u < e.length; u++)
                for (let a = 0; a < t.length; a++) n[u + a] ^= o.mul(e[u], t[a]);
            return n
        }, r.mod = function(e, t) {
            let n = new Uint8Array(e);
            for (; n.length - t.length >= 0;) {
                const u = n[0];
                for (let s = 0; s < t.length; s++) n[s] ^= o.mul(t[s], u);
                let a = 0;
...
function de() {
    return _t || (_t = 1, (function(r) {
        ...
        function s(h, f, N) {
            for (let M = 1; M <= 40; M++)
                if (f <= r.getCapacity(M, N, h)) return M
        }
        ...
        function B(h, f) {
            for (let N = 1; N <= 40; N++)
                if (d(h, N) <= r.getCapacity(N, f, t.MIXED)) return N
        }
        r.from = function(f, N) {
            ...
            typeof M > "u" && (M = t.BYTE);
            const S = o.getSymbolTotalCodewords(f),
                A = i.getTotalCodewordsCount(f, N),
                P = (S - A) * 8;
            ...
            let N = f << 12;
            for (; o.getBCHDigit(N) - a >= 0;) N ^= u << o.getBCHDigit(N) - a;
            return f << 12 | N
        }
    })(ot)), ot
}
        ...
        var o = {
            single_source_shortest_paths: function(i, e, t) {
                ...
                for (var s, c, d, B, h, f, N, M, S; !a.empty();) {
                    ...
                    for (d in h) h.hasOwnProperty(d) && (f = h[d], N = B + f, M = u[d], S = typeof u[d] > "u", (S || M > N) && (u[d] = N, a.push(d, N), n[d] = c))
                }
...
function ye() {
    return Jt || (Jt = 1, (function(r) {
        ...
        function B(A) {
            const P = d(u.NUMERIC, o.NUMERIC, A),
                T = d(u.ALPHANUMERIC, o.ALPHANUMERIC, A);
            let p, b;
            return a.isKanjiModeEnabled() ? (p = d(u.BYTE, o.BYTE, A), b = d(u.KANJI, o.KANJI, A)) : (p = d(u.BYTE_KANJI, o.BYTE, A), b = []), P.concat(T, p, b).sort(function(E, C) {
            ...
            for (let g = 0; g < A.length; g++) {
                ...
                for (let l = 0; l < E.length; l++) {
                    ...
                    for (let y = 0; y < b.length; y++) {
                        const w = b[y];
                        T[w] && T[w].node.mode === R.mode ? (p[w][m] = h(T[w].lastCount + R.length, R.mode) - h(T[w].lastCount, R.mode), T[w].lastCount += R.length) : (T[w] && (T[w].lastCount = R.length), p[w][m] = h(R.length, R.mode) + 4 + o.getCharCountIndicator(R.mode, P))
                    }
                }
                b = C
            }
            for (let g = 0; g < b.length; g++) p[b[g]].end = 0;
            return {
                map: p,
                table: T
            }
        ...
        function S(A, P) {
            let T;
            const p = o.getBestModeForData(A);
            if (T = o.from(P, p), T !== o.BYTE && T.bit < p.bit) throw new Error('"' + A + '" cannot be encoded with mode ' + o.toString(T) + `.
 Suggested mode is: ` + o.toString(p));
        ...
        r.fromArray = function(P) {
            ...
            const p = B(P, a.isKanjiModeEnabled()),
                b = N(p),
                g = M(b, T),
                E = s.find_path(g.map, "start", "end"),
                C = [];
            for (let l = 1; l < E.length - 1; l++) C.push(g.table[E[l]].node);
            return r.fromArray(f(C))
        }, r.rawSplit = function(P) {
            return r.fromArray(B(P, a.isKanjiModeEnabled()))
        }
    })(at)), at
}
var Yt;
...
function Be() {
    ...
    function f(g, E) {
        ...
        for (let R = 0; R < l.length; R++) {
            ...
            for (let w = -1; w <= 7; w++)
                if (!(m + w <= -1 || C <= m + w))
                    for (let I = -1; I <= 7; I++) y + I <= -1 || C <= y + I || (w >= 0 && w <= 6 && (I === 0 || I === 6) || I >= 0 && I <= 6 && (w === 0 || w === 6) || w >= 2 && w <= 4 && I >= 2 && I <= 4 ? g.set(m + w, y + I, !0, !0) : g.set(m + w, y + I, !1, !0))
        }
    }
    ...
    function M(g, E) {
        ...
        for (let l = 0; l < C.length; l++) {
            const R = C[l][0],
                m = C[l][1];
            for (let y = -2; y <= 2; y++)
                for (let w = -2; w <= 2; w++) y === -2 || y === 2 || w === -2 || w === 2 || y === 0 && w === 0 ? g.set(R + y, m + w, !0, !0) : g.set(R + y, m + w, !1, !0)
        }
    }

    function S(g, E) {
        const C = g.size,
            l = c.getEncodedBits(E);
        let R, m, y;
        for (let w = 0; w < 18; w++) R = Math.floor(w / 3), m = w % 3 + C - 8 - 3, y = (l >> w & 1) === 1, g.set(R, m, y, !0), g.set(m, R, y, !0)
    }

    function A(g, E, C) {
        const l = g.size,
            R = d.getEncodedBits(E, C);
        let m, y;
        for (m = 0; m < 15; m++) y = (R >> m & 1) === 1, m < 6 ? g.set(m, 8, y, !0) : m < 8 ? g.set(m + 1, 8, y, !0) : g.set(l - 15 + m, 8, y, !0), m < 8 ? g.set(8, l - m - 1, y, !0) : m < 9 ? g.set(8, 15 - m - 1 + 1, y, !0) : g.set(8, 15 - m - 1, y, !0);
        g.set(l - 8, 8, 1, !0)
    }
    ...
    function P(g, E) {
        ...
        for (let w = C - 1; w > 0; w -= 2)
            for (w === 6 && w--;;) {
                for (let I = 0; I < 2; I++)
                    if (!g.isReserved(R, w - I)) {
                        let v = !1;
                        y < E.length && (v = (E[y] >>> m & 1) === 1), g.set(R, w - I, v), m--, m === -1 && (y++, m = 7)
                    }
                if (R += l, R < 0 || C <= R) {
                    R -= l, l = -l;
                    break
    ...
    function T(g, E, C) {
        ...
        const R = r.getSymbolTotalCodewords(g),
            m = a.getTotalCodewordsCount(g, E),
            y = (R - m) * 8;
        for (l.getLengthInBits() + 4 <= y && l.put(0, 4); l.getLengthInBits() % 8 !== 0;) l.putBit(0);
    ...
    function p(g, E, C) {
        const l = r.getSymbolTotalCodewords(E),
            R = a.getTotalCodewordsCount(E, C),
            m = l - R,
            y = a.getBlocksCount(E, C),
            w = l % y,
            I = y - w,
            v = Math.floor(l / y),
            V = Math.floor(m / y),
        ...
        let Bt = 0,
            D, q;
        for (D = 0; D < j; D++)
            for (q = 0; q < y; q++) D < H[q].length && (O[Bt++] = H[q][D]);
        for (D = 0; D < Et; D++)
            for (q = 0; q < y; q++) O[Bt++] = yt[q][D];
        return O
    }

    function b(g, E, C, l) {
        let R;
        if (Array.isArray(g)) R = h.fromArray(g);
            ...
            R = h.fromString(g, v || 40)
        } else throw new Error("Invalid data");
        const m = c.getBestVersionForData(R, C);
...
`);
        const y = T(E, C, R),
            w = r.getSymbolSize(E),
            I = new e(w);
        return f(I, E), N(I), M(I, E), A(I, C, 0), E >= 7 && S(I, E), P(I, y), isNaN(l) && (l = u.getBestMask(I, A.bind(null, I, C))), u.applyMask(l, I), A(I, C, l), {
            modules: I,
            version: E,
    ...
    return $.create = function(E, C) {
        if (typeof E > "u" || E === "") throw new Error("No input text");
        let l = o.M,
            R, m;
        return typeof C < "u" && (l = o.from(C.errorCorrectionLevel, o.M), R = c.from(C.version), m = u.from(C.maskPattern), C.toSJISFunc && r.setToSJISFunction(C.toSJISFunc)), b(E, R, l, m)
    }, $
}
var ht = {},
    mt = {},
...
function pe() {
    return Ot || (Ot = 1, (function(r) {
        ...
        r.render = function(n, u, a) {
            ...
            const d = o.getImageWidth(n.modules.size, s),
                B = c.getContext("2d"),
...
function Re() {
    ...
    function e(t, n, u) {
        ...
        for (let B = 0; B < t.length; B++) {
            const h = Math.floor(B % n),
                f = Math.floor(B / n);
            !h && !c && (c = !0), t[B] ? (d++, B > 0 && h > 0 && t[B - 1] || (a += c ? i("M", h + u, .5 + f + u) : i("m", s, 0), s = 0, c = !1), h + 1 < n && t[B + 1] || (a += i("h", d), d = 0)) : s++
        }
        return a
    }
    return wt.render = function(n, u, a) {
        const s = r.getOptions(u),
            c = n.modules.size,
            d = n.modules.data,
            B = c + s.margin * 2,
            h = s.color.light.a ? "<path " + o(s.color.light, "fill") + ' d="M0 0h' + B + "v" + B + 'H0z"/>' : "",
            f = "<path " + o(s.color.dark, "stroke") + ' d="' + e(d, c, s.margin) + '"/>',
            N = 'viewBox="0 0 ' + B + " " + B + '"',
            S = '<svg xmlns="http://www.w3.org/2000/svg" ' + (s.width ? 'width="' + s.width + '" height="' + s.width + '" ' : "") + N + ' shape-rendering="crispEdges">' + h + f + `</svg>
`;
...
function Ae() {
    ...
    function t(n, u, a, s, c) {
        const d = [].slice.call(arguments, 1),
            B = d.length,
            h = typeof d[B - 1] == "function";
        if (!h && !r()) throw new Error("Callback required as last argument");
        if (h) {
